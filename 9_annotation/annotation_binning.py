#!/usr/bin/env python3
"""Calculate and plot taxonomic breadth from current binning annotations."""
...
DATASET_INPUTS = {
    "Human Gut": {
        "dataset": "hg",
        "results": "results",
        "metahict_annotation": ROOT / "hg/9_annotation/results/classify",
    },
    "Pig Gut": {
        "dataset": "pig",
        "results": "results",
        "metahict_annotation": ROOT / "pig/9_annotation/results/classify",
    },
    "Wastewater": {
        "dataset": "ww",
        "results": "results",
        "metahict_annotation": ROOT / "ww/9_annotation/results/classify",
    },
}
...
def load_annotations(annotation_dir: Path) -> dict[str, str]:
    """Load bacterial and archaeal GTDB-Tk classifications by bin ID."""
    ...
    for marker in ("bac120", "ar53"):
        ...
        for bin_id, classification in zip(
            ...
            # GTDB-Tk output names may omit the dot in ``bin.1`` or retain a
            # FASTA variant suffix added during annotation.
...
def load_medium_quality_bins(stats_file: Path) -> set[str]:
    """Return bins with completeness ≥50% and contamination <10%."""
...
def calculate_current_data() -> dict[str, dict[str, dict[str, list[int]]]]:
    """Calculate pairwise taxon intersections from the selected result trees."""
    output = {}
    for dataset_label, config in DATASET_INPUTS.items():
        dataset = config["dataset"]
        result_name = config["results"]
        result_root = ROOT / dataset / "6_binning" / result_name / "metahit"
...
# Replace the legacy manuscript constants above with values calculated from files.
...
def save_single_dataset(dataset: str, stem: str) -> None:
    configure_style()
    fig, axes = plt.subplots(1, len(COMPARISONS), figsize=(24, 8))
    draw_dataset_row(axes, dataset)
    fig.tight_layout()
    fig.savefig(SCRIPT_DIR / f"{stem}.pdf", dpi=300, bbox_inches="tight")
...
def plot() -> None:
    ...
    fig.subplots_adjust(left=0.09, right=0.99, top=0.98, bottom=0.08, hspace=0.25, wspace=0.24)
    fig.savefig(SCRIPT_DIR / "annotation_binning.pdf", format="pdf", bbox_inches="tight")
    fig.savefig(SCRIPT_DIR / "annotation_binning.png", format="png", dpi=300, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    save_single_dataset("Human Gut", "annotation_plot_hg")
    save_single_dataset("Pig Gut", "annotation_plot_pig")
    save_single_dataset("Wastewater", "annotation_plot_ww")
    plot()
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_hg.pdf'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_hg.png'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_pig.pdf'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_pig.png'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_ww.pdf'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_plot_ww.png'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_binning.pdf'}")
    print(f"Wrote {SCRIPT_DIR / 'annotation_binning.png'}")
